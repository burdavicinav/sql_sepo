...
--          Dbms_Output.put_line(j.key_ || ' ' || k.perehkey);
  ...
  IS
    ...
    l_doccode NUMBER;
    l_state NUMBER;
    l_correct_docname VARCHAR2(1000);
  BEGIN
    DELETE FROM sepo_fixture_af_result_temp;

    FOR i IN (
      SELECT
        *
      FROM
        v_sepo_fixture_af_load
      WHERE
          doc_id = p_iddoc

    ) LOOP

      l_correct_docname := regexp_replace(p_docname, '^\d+_', '');
      ...
      FROM
        ...
        ON
            dp.code = a.document
      WHERE
          a.businessobj = i.bocode
        AND
          dp.filename = l_correct_docname;

      IF l_attach_count = 0 THEN
        pkg_sepo_documents.create_document(l_correct_docname, p_hash, p_data, l_doccode);
        pkg_sepo_documents.attach_document(i.bocode, l_doccode, i.id_file_group);
      ...
      INSERT INTO sepo_fixture_af_result_temp (
        id, iddoc, filename,
        objsign, objtype, revision, state
      )
      VALUES (
        sq_sepo_fixture_af_result_temp.NEXTVAL, p_iddoc, p_docname,
        i.designatio, i.botypename, i.revision, l_state
      );
