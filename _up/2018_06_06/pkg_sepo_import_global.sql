...
--    DELETE FROM omp_objects WHERE code = l_array(i);
  ...
  BEGIN
    ...
    pkg_sepo_attr_operations.addAttr('O_BO');
    ...
    FOR i IN (
      ...
      pkg_sepo_attr_operations.setValue('O_BO', i.o_vo);
  ...
  PROCEDURE ImportLog (p_msg VARCHAR2)
  AS
    pragma autonomous_transaction;
    begin
...
--    Dbms_Output.put_line(p_attr_name);
  ...
  BEGIN
    ...
    DELETE FROM sepo_std_scheme_attrs_temp;
    ...
    FOR i IN (
      ...
      l_foxpro_attr := createstdfixattr(
        i.name,
        i.shortname,
        i.type_,
        NULL
    ...
    l_rel_attr_id := createstdfixattr(
      l_rel_attr_name,
      l_rel_attr_name,
      10,
      l_rel_enum_id
    ...
    -- добавление атрибутов foxpro в схему по умолчанию
    FOR i IN (
      SELECT
        attrcode
      FROM
    ...
    -- Table
    l_attr_id := createstdfixattr('Table', 'Table', 1, NULL);
    -- RecKey
    l_attr_id := createstdfixattr('RecKey', 'RecKey', 3, NULL);
    -- TBLKey
    l_attr_id := createstdfixattr('TBLKey', 'TBLKey', 3, NULL);
    -- О_ВО
    l_attr_id := createstdfixattr('О_ВО', 'О_ВО', 1, NULL);

    INSERT INTO sepo_std_scheme_attrs_temp (
      id_table, attrcode
    )
    SELECT
      a.id_table,
...
--    Dbms_Output.put_line(p_attr_name);
    ...
    ELSE
      IF l_attr_type != p_attr_type THEN
  ...
  BEGIN
    ...
    -- Table
    l_attr_id := createoldfixattr('Table', 'Table', 1);
    -- RecKey
    l_attr_id := createoldfixattr('RecKey', 'RecKey', 3);
    -- TBLKey
    l_attr_id := createoldfixattr('TBLKey', 'TBLKey', 3);
    -- О_ВО
    l_attr_id := createoldfixattr('О_ВО', 'О_ВО', 1);
...
--        Dbms_Output.put_line(j.attr_name || ' ' || j.attr_value);
  ...
  BEGIN
    ...
    pkg_sepo_attr_operations.addAttr('Table');
    pkg_sepo_attr_operations.addAttr('RecKey');
    pkg_sepo_attr_operations.addAttr('TBLKey');
    ...
    FOR i IN (
      ...
      IF l_fixcode IS NULL THEN
        ...
        pkg_sepo_attr_operations.setValue('Table', i.f_table);
        pkg_sepo_attr_operations.setValue('RecKey', i.reckey);
        pkg_sepo_attr_operations.setValue('TBLKey', i.tblkey);
        pkg_sepo_attr_operations.setValue('О_ВО', i.designation);
        pkg_sepo_attr_operations.insertAttrs(l_bo_code);
