...
IS
  ...
  PROCEDURE insertAttrs(p_socode NUMBER);
...
IS
  ...
  BEGIN
    ...
    FOR i IN 1..pkg_listData.Count LOOP
      IF pkg_listData(i).nameAttr = p_shortname THEN
        IF pkg_listData(i).typeAttr NOT IN (2,3) THEN
          Raise_Application_Error (
  ...
  IS
  BEGIN
    IF pkg_listData(p_index).typeAttr NOT IN (2,3) THEN
      Raise_Application_Error (
  ...
  IS
    l_sql VARCHAR2(1000);
  BEGIN
    l_sql := 'insert into obj_attr_values_' || pkg_obj_type || '(socode,';

    FOR i IN 1..pkg_listData.Count LOOP
      l_sql := l_sql || 'A_' || pkg_listData(i).codeAttr;
      IF i < pkg_listData.Count THEN
        l_sql := l_sql || ',';
      END IF;
  ...
  BEGIN
    ...
    Dbms_Sql.parse(l_cursor, pkg_insertSql, dbms_sql.native);
    Dbms_Sql.bind_variable(l_cursor, ':socode', p_socode);

    FOR l_index IN 1..l_params.Count() LOOP
      IF l_params(l_index).type_ IN (2,3) THEN
